        ...
        with open(ref_file) as ff:
            ...
            for line in f:
                ...
                try:
                    ...
                    print(f"[WARNING] Skipping invalid line: {line.strip()} - Error: {e}")
                    continue
...
def parse_sumstats(ref_dict, vld_dict, sst_file, pop, n_subj):
    ...
    for (ii,snp) in enumerate(ref_dict['SNP']):
        if snp in sst_eff:
            ...
            # 先確認頻率欄位是否存在
            frq_key = 'FRQ_' + pop.upper()
            flp_key = 'FLP_' + pop.upper()
    
            if frq_key in ref_dict:
                frq_values = ref_dict[frq_key]
            else:
                # 如果沒有對應族群的頻率欄位，改用 MAF（或其他預設欄位）
                frq_values = ref_dict.get('MAF', [0]*len(ref_dict['SNP']))
    
            if flp_key in ref_dict:
                flp_values = ref_dict[flp_key]
            else:
                # 如果沒有 FLP，也改用0代替
                flp_values = [0]*len(ref_dict['SNP'])
    
            if (snp, a1, a2) in comm_snp or (snp, mapping[a1], mapping[a2]) in comm_snp:
                sst_dict['FRQ'].append(frq_values[ii])
                sst_dict['FLP'].append(flp_values[ii])
            elif (snp, a2, a1) in comm_snp or (snp, mapping[a2], mapping[a1]) in comm_snp:
                sst_dict['FRQ'].append(1 - frq_values[ii])
                sst_dict['FLP'].append(-1 * flp_values[ii])
